...
| `kbrd plugin add <marketplace/plugin[@version]>` | Resolve a Lua, static-asset, or mixed plugin into the current board's `kbrd.plugins.lock` and cache its verified content. |
...
| `kbrd plugin starter list [marketplace/plugin]` | List board starter kits distributed by enabled locked plugins. |
| `kbrd plugin starter apply <marketplace/plugin> <starter> [--target dir] [--force]` | Copy a verified starter kit into a board with conflict checks. |
...
| `Ctrl+P` | Switch board |
| `l` | Switch runtime layer declared by a board plugin or `.kbrd.lua` |
...
[companion]
shortcut = "command+shift+k" # global config only; restart the companion after changing
...
- `kbrd.layer{...}` — declare exclusive plugin or folder-local runtime layers of commands, hooks, eval functions, timers, async/HTTP work, and virtual columns, with optional `before_activate` / `before_deactivate` lifecycle callbacks; switch them with `l` and see the active layer in the header.
- `kbrd.command(...)` — register a custom command (appears in the `x` menu; supports a Lua `visible(ctx)` predicate for per-card frontmatter filtering; shadows a shell command with the same id).
