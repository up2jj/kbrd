...
kbrd.command("viewer", "Viewer", function()
  kbrd.ui.viewer({title="Patch", content="+line", format="diff", line_numbers=true,
    actions={{id="apply",label="Apply",key="ctrl+a"}}})
end)`)
	h, err := New(defaultCfg(), &fakeAPI{}, nil, dir, "")
	if err != nil {
		t.Fatalf("load: %v", err)
	...
	h.CancelPending()
	viewer, err := h.RunCommand(h.Commands()[1].LuaRef, nil)
	if err != nil || viewer.Kind != UIKindViewer || viewer.Spec.Format != "diff" || viewer.Spec.Content != "+line" {
		t.Fatalf("viewer = (%+v, %v)", viewer, err)
	}
}

func TestPhaseFourWidgetValidationErrors(t *testing.T) {
	tests := []struct {
		name string
		body string
		want string
